...
private:
	...
	std::vector<std::string> allowed_hubble_type{"E0", "E1", "E2" , "E3" , "E4" , "E5" , "E6" , "E7", "S0", "Sa", "Sb", "Sc", "SBa", "SBb", "SBc", "Irr"};
	double redshift;
	double total_mass;
...
public:
	...
	galaxy(std::string type, double z, double M_tot, double f): hubble_type(type), redshift(z), total_mass(M_tot), stellar_mass_fraction(f)
	{
	...
	void change_hubble_type(std::string type)
	{
		...
		if (std::find(allowed_hubble_type.begin(), allowed_hubble_type.end(), type) != allowed_hubble_type.end()) {
			hubble_type = type;
		}
		else {
	...
	void set_redshift(double z)
	{
		if (redshift < 0.0 || redshift > 10.0) {
			std::cerr << "Redshift must be between 0 and 10! Falling Back to default (0.0)." << std::endl;
			redshift = 0.0;
		} else {
			redshift = z;
		}
	...
	{
		if (!(std::find(allowed_hubble_type.begin(), allowed_hubble_type.end(), type) != allowed_hubble_type.end())) {
			std::cerr << "Wrong Hubble Type! Falling Back to default (Irr)." << std::endl;
			hubble_type = "Irr";
		} else {
			hubble_type = type;
		}
	...
	void set_total_mass(double total)
	{
		if (total < 1e7 || total > 1e12) {
			std::cerr << "Total Mass must be between 1e7 and 1e12! Falling Back to default (1e7)." << std::endl;
			total_mass = 1e7;
		} else {
			total_mass = total;
		}
	...
	void set_stellar_mass_fraction(double stellar_fraction)
	{
		if (stellar_fraction < 0.0 || stellar_fraction > 0.05) {
			std::cerr << "Stellar mass fraction must be between 0 and 0.05! Falling Back to default (0.0)" << std::endl;
			stellar_mass_fraction = 0.0;
		} else {
...
void galaxy::print()
{
	...
	if (!satellites.empty()) { //if non-zero satellites
		...
		for (std::vector<galaxy>::iterator it = satellites.begin(); it != satellites.end(); ++it) {
			std::cout << std::endl;
...
int main()
{
	...
	//declaring wrong values (important: galaxy still defined but with default values instead)
	std::cout << "Demonstration of declaring wrong values for galaxy:" << std::endl;
	...
	std::cout << "Demonstration of changing Hubble type to a wrong value, then to a correct value:" << std::endl;
	...
	std::cout << "Demonstration of Adding Satellite Galaxies and printing them:" << std::endl;
